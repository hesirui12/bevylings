...
fn wire_systems(app: &mut App) {
    // BUG: 想表达"两个条件都满足才升级"（AND，应该用 and_then），
    // 这里却写成了"至少一个满足就升级"（OR，or_else）。
    app.add_systems(Update, level_up.run_if(has_energy.or_else(below_max)));
    app.add_systems(Update, recover.run_if(not(has_energy)));
}
