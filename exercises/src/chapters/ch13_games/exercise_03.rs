...
//! 运行 `bevylings run 1303` 查看现象（目前会编译报错），
//! 改正后运行 `bevylings test 1303` 让测试通过。
//!
...
fn update_loading(
    ...
    if loading_data.remaining > 0 {
        ...
        *loading_state = LoadingState::LevelLoading;
        ...
        // BUG: 全部加载完成后应该把状态改成 `LevelReady`（就绪），
        // 这里却忘了用 `*` 解引用 `ResMut`，直接给句柄赋值，类型对不上。
...
// 提示：
// 1. 编译错误会提示类型不匹配：给 `ResMut<LoadingState>` 赋了一个 `LoadingState`。
// 2. `ResMut<T>` 只是句柄，想改它包着的值必须先 `*` 解引用。
// 3. 修改后运行 `bevylings test 1303`，两个测试全绿就过关。
