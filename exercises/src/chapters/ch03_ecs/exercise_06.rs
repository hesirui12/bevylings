...
fn list_crews(
    ...
    for (ship_id, ship_name, captain) in &ships {
        // BUG: 顺着"船自己的编号"去查船长，找到的当然是船本身。
        // 船恰好也有名字，结果就变成了"宝船 的船长是 宝船"。
        // 应该顺着 captain.0（船长编号）去查。
        if let Ok(captain_name) = captains.get(ship_id) {
